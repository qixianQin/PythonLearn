...
from zhihuspider.setting import MONGO_URI, PROJECT_DIR
from zhihuspider.items import ZhihuPeopleItem, ZhihuRealtionItem
from zhihuspider.tools.async import download_pic
...
class ZhihuspiderPipeline(object):
    ...
    def _process_people(self,item):
    	...
    	if image_url and zhihu_id:
    		...
    		download_pic.delay(iamge_url, image_path)
    ...
    def _process_relation(self,item):
    	...
    	data = collection.find_one({'zhihu_id':item['zhihu_id'],'user_type':item['user_type']})
    	if not data:
    		...
    		collection.update({'zhihu_id':item['zhihu_id'], 'user_type':item['user_type']}, data)
    ...
    def process_item(self, item, spider):
    	...
    	elif isinstance(item, ZhihuRealtionItem):
